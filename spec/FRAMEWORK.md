...
- Co-signer re-execution (`replay.py`): before a jailed run, the co-signer copies the workdir for itself; after it, the co-signer replays the record's calls against a fresh copy of that snapshot, mounted at the workdir's own path so the command sees the same paths, $PWD and files it saw the first time, traced by its own invocation of the jail and read by its own parser. It signs only if the replay's effects are the record's. A snapshot it did not take, or of another workdir, is refused. A forged trace (lines dropped by the executor or its jail) is caught here, because the replay does not read it
...
- Observation soundness (the critic proves the Vow against the effects `observation.py` reports, not against what a plan actually does). Default-deny scope effects (read/write_outside_workdir, exec_unvetted, network_unlisted) now catch all seven exploit shapes in `tests/test_observation_gaps.py`, up from 0 of 7; `CAUGHT_BY` there pins which effect catches each one. What remains open:
  ...
  - the co-signer re-derives a run's effects from its record with its own trace parser, so a guard that misreports what the record shows, or whose parser misreads it, is refused; but it trusts the raw trace. What the two parsers share is the event-to-effect mapping (`jail.effects_of`, the Vow's vocabulary), and a failed open is resolved through symlinks as the co-signer finds the filesystem, which is the same moment only when it co-signs right after the run. A co-signer built with `reexecute=True` (the guarded agent's, and the demo's when a jail is present) does not trust the raw trace either: it copies the workdir before the run and replays the plan from that copy under its own trace (`replay.py`), so forged trace lines are caught. What that replay still shares with the executor is the machine (the same bwrap and strace binaries, the same kernel), and a command whose effects vary from run to run is refused rather than signed. Without `reexecute`, forged trace lines are not caught. An action with no run behind it carries no record, and there the guard's effect set is still the only observation
